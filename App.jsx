  ...
  return (
    <div style={{minHeight:"100vh",background:"#010812",backgroundImage:"radial-gradient(ellipse at 20% 15%,rgba(0,255,136,0.04) 0%,transparent 50%),radial-gradient(ellipse at 80% 80%,rgba(0,100,255,0.03) 0%,transparent 50%)",fontFamily:"'Courier New',monospace",color:"#cce0ff"}}>
      ...
      <div style={{maxWidth:760,margin:"0 auto",padding:"12px 12px 24px",display:"flex",flexDirection:"column",gap:10}}>
        ...
        {/* TEAM SELECTOR */}
        <div style={{...c.card}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 36px 1fr",gap:10,alignItems:"end",marginBottom:12}}>
            <div>
              <span style={c.lbl}>🏠 Home Team</span>
              <select value={homeSlug} onChange={e=>setHomeSlug(e.target.value)} style={{...ss,width:"100%"}}>
                {Object.entries(TEAMS.reduce((g,t)=>({...g,[t.league]:[...(g[t.league]||[]),t]}),{})).map(([lg,teams])=>(
            ...
            <div style={{textAlign:"center",fontSize:11,color:"#2a4060",fontWeight:700,paddingBottom:8}}>VS</div>
            <div>
              <span style={c.lbl}>✈️ Away Team</span>
              <select value={awaySlug} onChange={e=>setAwaySlug(e.target.value)} style={{...ss,width:"100%"}}>
                {Object.entries(TEAMS.reduce((g,t)=>({...g,[t.league]:[...(g[t.league]||[]),t]}),{})).map(([lg,teams])=>(
                  <optgroup key={lg} label={lg}>
                    {teams.map(t=><option key={t.slug} value={t.slug}>{t.name}</option>)}
                  </optgroup>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:12}}>
            {[["home","🏠 Odds Home"],["draw","🤝 Odds Draw"],["away","✈️ Odds Away"]].map(([k,lbl])=>(
              <div key={k}>
                <span style={c.lbl}>{lbl}</span>
                <input type="number" step="0.05" min="1" value={marketOdds[k]}
                  onChange={e=>setMarketOdds(p=>({...p,[k]:parseFloat(e.target.value)||1}))}
                  style={{...ss,width:"100%",padding:"7px 10px"}}/>
              </div>
            ))}
          </div>

          <button onClick={runAnalysis} disabled={homeSlug===awaySlug||loadState==="loading"} style={{
            width:"100%",padding:"12px",borderRadius:10,border:"none",fontFamily:"'Courier New',monospace",
            background:homeSlug===awaySlug||loadState==="loading"?"rgba(255,255,255,0.04)":"linear-gradient(135deg,#00ff88,#00cc88)",
            color:homeSlug===awaySlug||loadState==="loading"?"#444":"#001a0d",
            fontWeight:900,fontSize:13,letterSpacing:3,cursor:homeSlug===awaySlug||loadState==="loading"?"not-allowed":"pointer",
            boxShadow:loadState!=="loading"&&homeSlug!==awaySlug?"0 0 24px rgba(0,255,136,0.2)":"none",
            display:"flex",alignItems:"center",justifyContent:"center",gap:8
          }}>
            {loadState==="loading" ? <><Spinner/> FETCHING FBREF DATA...</> : "🔮 ANALISIS DENGAN DATA FBREF"}
          </button>

          {dataSource && (
            <div style={{marginTop:8,fontSize:10,textAlign:"center",color:dataSource.includes("Live")?"#00ff88":"#ff9944"}}>
              {dataSource}
            </div>
          )}
        </div>
        ...
        {loadState==="done" && (
          <>
            <div style={{display:"flex",gap:6}}>
              {[["predict","🎯 Prediksi"],["stats","📊 FBref Stats"],["chat","💬 Chat AI"]].map(([t,l])=>(
                <button key={t} onClick={()=>setTab(t)} style={{flex:1,padding:"8px",borderRadius:8,border:"none",
                  background:tab===t?"rgba(0,255,136,0.10)":"rgba(255,255,255,0.03)",
                  color:tab===t?"#00ff88":"#445566",fontSize:11,cursor:"pointer",
                  fontFamily:"'Courier New',monospace",borderBottom:tab===t?"2px solid #00ff88":"2px solid transparent"}}>
                  {l}
                </button>
              ))}
            </div>
            ...
            {tab==="predict" && prediction && (
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {/* Verdict */}
                <div style={{...c.card,textAlign:"center",borderColor:`${prediction.confColor}30`}}>
                  <div style={{fontSize:9,color:"#3a5570",letterSpacing:3,marginBottom:6}}>PREDIKSI ENGINE — FBref + Market Blend</div>
                  <div style={{fontSize:9,color:"#3a5570",marginBottom:14}}>
                    {homeTeam?.name} <span style={{color:"#2a4060"}}>vs</span> {awayTeam?.name}
                  </div>
                  <ProbBar label={`🏠 ${homeTeam?.name}`} pct={prediction.homeWin} color="#4488ff" sub={`Odds: ${marketOdds.home}`}/>
                  <ProbBar label="🤝 Draw"                 pct={prediction.draw}    color="#ffcc00" sub={`Odds: ${marketOdds.draw}`}/>
                  <ProbBar label={`✈️ ${awayTeam?.name}`} pct={prediction.awayWin} color="#00ff88" sub={`Odds: ${marketOdds.away}`}/>
                  <div style={{marginTop:10,display:"flex",justifyContent:"center",gap:8,flexWrap:"wrap"}}>
                    <Badge text={`Confidence: ${prediction.confidence}`} color={prediction.confColor}/>
                    <Badge text={`xG ${prediction.xg.home} - ${prediction.xg.away}`} color="#aabbcc"/>
                  </div>
                </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
                  {[
                    {l:"BTTS",    v:`${prediction.btts}%`,    c:"#ffcc00"},
                    {l:"Over 2.5",v:`${prediction.over25}%`,  c:"#ff9944"},
                    {l:"Over 3.5",v:`${prediction.over35}%`,  c:"#ff6b6b"},
                    {l:"Source",  v:"FBref",                   c:"#00aaff"},
                  ].map(({l,v,c_})=>(
                    <div key={l} style={{...c.card,textAlign:"center",padding:"10px 6px"}}>
                      <div style={{fontSize:8,color:"#3a5570",letterSpacing:1}}>{l}</div>
                      <div style={{fontSize:13,color:"#e0eeff",fontWeight:800,marginTop:3}}>{v}</div>
                    </div>
                  ))}
                </div>
                ...
                {/* Skor */}
                <div style={c.card}>
                  <span style={c.lbl}>Probabilitas skor</span>
                  <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                    {prediction.topScores.map(({score,prob},i)=>(
                      <div key={score} style={{padding:"4px 12px",borderRadius:20,fontSize:12,
                        background:i===0?"rgba(0,255,136,0.10)":"rgba(255,255,255,0.04)",
                        border:`1px solid ${i===0?"rgba(0,255,136,0.30)":"rgba(255,255,255,0.08)"}`,
                        color:i===0?"#00ff88":"#889aaa",fontWeight:i===0?700:400}}>
            ...
            {tab==="stats" && hs && as_ && (
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {/* Header comparison */}
                <div style={{...c.card}}>
                  <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",gap:4,marginBottom:12,textAlign:"center"}}>
                    <div style={{color:"#4488ff",fontSize:12,fontWeight:700}}>{homeTeam?.name}</div>
                    <div style={{fontSize:9,color:"#3a5570",padding:"0 8px"}}>FBREF STATS</div>
                    <div style={{color:"#00ff88",fontSize:12,fontWeight:700}}>{awayTeam?.name}</div>
                  </div>

                  {[
                    {label:"xG / Game",     hv:hs.avgXgLast5, av:as_.avgXgLast5, higher:true},
                    {label:"xGA / Game",    hv:hs.avgXgaLast5,av:as_.avgXgaLast5,higher:false},
                    {label:"Form Pts/15",   hv:hs.formPts,    av:as_.formPts,     higher:true},
                    {label:"Press Succ %",  hv:hs.pressureSucc,av:as_.pressureSucc,higher:true},
                    {label:"PSxG-GA (GK)",  hv:hs.psxgDiff,   av:as_.psxgDiff,    higher:true},
                    {label:"Pass Acc %",    hv:hs.passPct,    av:as_.passPct,     higher:true},
                  ].map(({label,hv,av,higher})=>(
                    <StatRow key={label} label={label} homeVal={typeof hv==="number"?hv.toFixed(1):hv} awayVal={typeof av==="number"?av.toFixed(1):av} higherBetter={higher}/>
                  ))}
                </div>

                {/* Form */}
                {[{team:homeTeam,form:hs.form,color:"#4488ff"},{team:awayTeam,form:as_.form,color:"#00ff88"}].map(({team,form,color})=>(
                  <div key={team?.slug} style={c.card}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                      <span style={{fontSize:12,fontWeight:700,color}}>{team?.name}</span>
                      <span style={c.lbl}>FORM 5 LAGA</span>
                    </div>
                    <div style={{display:"flex",gap:6}}>
                ...
                {/* Data info */}
                <div style={{...c.card,fontSize:10,color:"#3a5570",lineHeight:2}}>
                  <span style={c.lbl}>📡 Sumber data</span>
                  <div>✅ xG, xGA, PSxG-GA — <strong style={{color:"#cce0ff"}}>StatsBomb via FBref</strong></div>
                  <div>✅ Pressing success %, Pass accuracy — FBref advanced stats</div>
                  <div>✅ Form per laga + match log — FBref match logs</div>
                  <div>✅ Market odds — bookmaker aggregate (input manual)</div>
                  <div style={{marginTop:4,color: dataSource?.includes("Live")?"#00ff88":"#ff9944"}}>{dataSource}</div>
                </div>
              </div>
            )}
            ...
            {tab==="chat" && (
              <div style={{...c.card,display:"flex",flexDirection:"column"}}>
                <div style={{fontSize:9,color:"#2a4060",letterSpacing:2,marginBottom:12}}>
                  💬 CHAT — CONTEXT: {homeTeam?.name?.toUpperCase()} VS {awayTeam?.name?.toUpperCase()}
                </div>
                <div style={{overflowY:"auto",maxHeight:360,marginBottom:10}}>
                  {msgs.length===0&&!chatLoading&&<div style={{textAlign:"center",color:"#2a4060",fontSize:11,marginTop:30}}>Jalankan analisis dulu...</div>}
                  {msgs.map((m,i)=><ChatBubble key={i} msg={m}/>)}
                  {chatLoading&&<ChatBubble msg={{role:"ai"}} isTyping/>}
                  <div ref={bottomRef}/>
                </div>
                <div style={{display:"flex",gap:8}}>
                  <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendChat()}
                    placeholder="Tanya soal xG, pressing, GK quality, over/under..."
                    style={{flex:1,...ss,padding:"9px 12px"}}/>
                  <button onClick={sendChat} disabled={chatLoading||!input.trim()}
                    style={{padding:"9px 14px",borderRadius:8,border:"none",background:"rgba(0,255,136,0.10)",color:"#00ff88",cursor:"pointer",fontSize:15}}>→</button>
                </div>
                <div style={{display:"flex",gap:6,marginTop:8,flexWrap:"wrap"}}>
                  {["Apa arti PSxG-GA?","Siapa GK lebih baik?","Value bet mana?","Kenapa over 2.5 menarik?"].map(q=>(
                    <button key={q} onClick={()=>setInput(q)} style={{...ss,width:"auto",padding:"4px 9px",fontSize:10,color:"#445566"}}>{q}</button>
                  ))}
                </div>
              </div>
            )}
        ...
        {loadState==="idle" && (
          <div style={{...c.card,textAlign:"center",padding:32}}>
            ...
            <div style={{fontSize:13,color:"#445566",lineHeight:2}}>
              Pilih dua tim lalu klik <strong style={{color:"#00ff88"}}>ANALISIS</strong><br/>
              Data akan diambil langsung dari <strong style={{color:"#cce0ff"}}>FBref / StatsBomb</strong><br/>
              <span style={{fontSize:10,color:"#3a5570"}}>xG · PSxG-GA · Pressing · Pass Accuracy · Form · H2H</span>
            </div>
            {backendStatus==="offline" && (
              <div style={{marginTop:16,padding:"10px 16px",borderRadius:8,background:"rgba(255,153,68,0.08)",border:"1px solid rgba(255,153,68,0.25)",fontSize:11,color:"#ff9944"}}>
                ⚠️ Backend offline — pakai data fallback.<br/>
                Deploy backend ke Railway/Render dulu untuk data live FBref.
              </div>
            )}
          </div>
        )}
        ...
        {loadState==="error" && (
          <div style={{...c.card,textAlign:"center",padding:24,borderColor:"rgba(255,107,107,0.3)"}}>
            <div style={{color:"#ff6b6b",fontSize:13}}>❌ Terjadi error saat fetch data.</div>
            <div style={{color:"#445566",fontSize:11,marginTop:6}}>Cek koneksi atau coba lagi.</div>
          </div>
        )}

        <div style={{textAlign:"center",fontSize:9,color:"#1a3050",letterSpacing:1}}>
          BOLA.AI v4 · FBref/StatsBomb · PREDIKSI PROBABILISTIK · BUKAN JAMINAN HASIL
        </div>
      </div>
    </div>
  );
